package rip.thecraft.brawl.util.imagemessage;
...
public class ImageMessage {
    ...
    public ImageMessage(String image) {
        File directory = new File(Brawl.getInstance().getDataFolder(), "ascii-art");
        if (!directory.exists() && directory.mkdir()) {
            Brawl.getInstance().getLogger().log(Level.INFO, "Created image directory");
        }
        ...
        if (bufferedImage == null) {
            Brawl.getInstance().getLogger().info("Attempting to copy resource '" + image + "' to plugin folder");
            try (InputStream input = Brawl.getInstance().getResource(directory.getName() + "/" + file.getName()); OutputStream output = new FileOutputStream(file)) {
                ByteStreams.copy(input, output);
                bufferedImage = ImageIO.read(input);
            } catch (IOException ex) {
                Brawl.getInstance().getLogger().log(Level.WARNING, "Failed to get resource for file '" + image + "'", ex);
            }
        }

        ChatColor[][] chatColors = toChatColorArray(bufferedImage, 8);
    ...
    public void broadcast() {
        for (String line : this.lines) {
            Brawl.getInstance().getServer().broadcastMessage(line);
        }
    }
